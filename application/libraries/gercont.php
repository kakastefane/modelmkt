	...
	*/
	public function listagem($params){
		...
		if (isset($params['buscar'])) {
            if ($params['tabela'] == 'conteudo')
                $where = "(titulo like '%{$params['buscar']}%' OR texto like '%{$params['buscar']}%')";
            else
                $where = "(titulo like '%{$params['buscar']}%' OR chamada like '%{$params['buscar']}%' OR texto like '%{$params['buscar']}%')";
            $this->db->where($where);
        }
